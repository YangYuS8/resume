...
**项目来源**：梦溪比特（鞍山）软件信息技术有限公司实习项目<br>
**接触技术**：Go、Gin、Gorm、MySQL、Docker、Git、接口联调、服务部署

- 参与用户、专家、视频、评论、收藏、浏览历史等模块的后端结构梳理与接口联调，配合前端验证接口返回、参数传递和业务流程。
- 协助处理数据库连接、端口占用、配置文件、依赖安装等本地运行问题，使用 Docker 整理并验证运行环境，沉淀运行说明和问题记录。
